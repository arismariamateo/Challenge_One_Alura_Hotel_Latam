package com.hotelaluralatam.views;
...
import com.hotelaluralatam.controller.HuespedesController;
import com.hotelaluralatam.controller.ReservasController;
...
//Código que permite mover la ventana por la pantalla según la posición de "x" y "y"
	...
	private void cargarReservas(){
		var reservas = reservasController.listar();
	...
	private void cargarHuespedes(){
		var huespedes = huespedesController.listar();
	...
	private void editarContenido(Integer index) {
		...
		if(index == 0) {
			...
			//	String fechaSalida = (String) modelo.getValueAt(tbReservas.getSelectedRow(), 2);
				...
				LocalDate fechaS;
				try {
					fechaE = LocalDate.parse(modelo.getValueAt(tbReservas.getSelectedRow(), 1).toString());
					fechaS = LocalDate.parse(modelo.getValueAt(tbReservas.getSelectedRow(), 2).toString());
				} catch (DateTimeParseException e) {
					JOptionPane.showMessageDialog(this, "La fecha modificada no es una fecha válida");
					return;
				}

				if (fechaE.until(fechaS, ChronoUnit.DAYS) <= 0) {
				...
				var reserva = new Reserva(id, fechaE, fechaS, valor, formaPago);
		...
		if(index == 1) {
			...
			//	String fechaNacimiento = (String) modeloH.getValueAt(tbHuespedes.getSelectedRow(), 3);
				...
				try {
					fechaN = LocalDate.parse(modeloH.getValueAt(tbHuespedes.getSelectedRow(), 3).toString());
				...
				int filasModificadas;
				var huesped = new Huesped(id, nombre, apellido, fechaN, nacionalidad, telefono, reserva);
	...
	private List<Reserva> buscar(String busqueda, String campo) {
		return reservasController.buscar(busqueda, campo);
	}
}
